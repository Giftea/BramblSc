...
This function creates your main key pair and encrypts it with the provided password.

### Parameters
- **`password`**: A required password to encrypt the main key pair.
- **`passphrase`**: An optional passphrase used during key generation. By default, no passphrase is used.
- **`mLen`**: The length of the mnemonic (word sequence) to generate. The default is 12 words.

If successful, this function returns a <ScaladocLink path="co/topl/brambl/wallet/WalletApi$$NewWalletResult.html"><code>NewWalletResult</code></ScaladocLink>, containing the encrypted key pair in a `VaultStore` instance and the mnemonic used to recover it. On failure, the function returns a <ScaladocLink path="co/topl/brambl/wallet/WalletApi$$WalletApiFailure.html"><code>WalletApiFailure</code></ScaladocLink>.
...
Here’s a simple example of creating a wallet without a passphrase and using the default mnemonic length (12 words):
...
This function generates a main key pair, encrypts it with the password, and persists it in the `WalletKeyApiAlgebra` initialized with the Wallet API.

### Parameters
- **`password`**: A password to encrypt the main key pair.
- **`passphrase`**: An optional passphrase used to generate the main key pair. The default is no passphrase.
- **`mLen`**: Mnemonic length (default is 12 words).
- **`name`**: The name for saving the wallet (default is "default").
- **`mnemonicName`**: The mnemonic’s save name (default is "mnemonic").
